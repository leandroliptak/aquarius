...
class InstagramAPI:
    ...
    def uploadPhoto(self, photo, caption=None, upload_id=None, is_sidecar=None, story=False):
        ...
        if response.status_code == 200:
            if story:
                config = self.configureStory(upload_id, photo, "")
            else:
                config = self.configure(upload_id, photo, caption)
            if config:
                self.expose()
        return False
    ...
    def configureStory(self, upload_id, photo, caption=''):

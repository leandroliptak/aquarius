...
class InstaBot:
    ...
    def __init__(self,
        ...
        # Desactivo el unfollow al salir
        ...
        if self.login_status:
            ...
            try:
                ...
                if follow.status_code == 200:
                    ...
                    log_string = "Followed: %s %s #%i." % (user_id, username,
                                                        self.follow_counter)
                    self.write_log(log_string)
        ...
        if self.login_status:
            ...
            try:
                ...
                if unfollow.status_code == 200:
                    ...
                    username = self.get_username_by_user_id(user_id=user_id)
                    log_string = "Unfollowed: %s %s #%i." % (user_id, username,
    ...
    def lean_like(self, media):
        ...
        self.write_log("Trying to like media: %s" % (media_id))
        like = self.like(media_id)

        if like != 0:
            self.write_log("  .. Returned code %i" % (like.status_code))

    def lean_auto_unfollow(self):
        checking = True
        while checking:
